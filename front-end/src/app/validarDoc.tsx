...
interface ValidacaoDocumentoProps {
  status: "idle" | "loading" | "success" | "error";
  mensagem?: string;
}

export function ValidacaoDocumento({
  status,
  mensagem,
}: ValidacaoDocumentoProps) {
  const renderIcon = () => {
    if (status === "loading") {
      return (
        <svg
    ...
    if (status === "success") {
      return <CheckCircle className="w-4 h-4 me-2 text-green-500" />;
    }

    if (status === "error") {
      return <XCircle className="w-4 h-4 me-2 text-red-500" />;
    }
          ...
          <ul className="max-w-md space-y-2 text-gray-500 list-inside dark:text-gray-400">
            {status === "loading" && (
              <li className="flex items-center">
                {renderIcon()}
                Validando com OCR e IA...
              </li>
            )}
            {mensagem && (
              <li className="ml-6 text-sm italic text-gray-400">
                Resultado: {mensagem}
              </li>
            )}
          </ul>
